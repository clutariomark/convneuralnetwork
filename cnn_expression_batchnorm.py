...
for k in range(10):
    ...
    test_prediction = []
    test_label = []
    chkptfile = os.path.join(log_dir, "model%d_batchnorm.ckpt" % k)
    ...
    filename = "train-%d_batchnorm.csv" % k
    with open(filename, "wb") as f:
        writer = csv.writer(f)
        writer.writerows(outlist)

    outtestlist = zip(test_acc, test_prediction, test_label)
    filename = "test-%d_batchnorm.csv" % k
    with open(filename, "wb") as f:
        writer = csv.writer(f)
...
with open("runtimes_batchnorm.csv") as f:
    writer = csv.writer(f)
